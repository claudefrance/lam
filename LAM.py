...
class LAMApp:
    ...
    def _encode_d50_name(self, text, length):
        ...
        result = []
        for c in text:
            result.append(D50_CHAR_TO_BYTE.get(c, 0))  # 0 = espace
        return result

    def _generate_80s_patch_name(self, length=18):
        """Génère un nom style 80s qui tient correctement dans la longueur demandée."""
        r = random.random()

        if r < 0.28:
            # Préfixe + Nom (avec espace)
            raw = f"{random.choice(PREFIXES_80S)} {random.choice(NOUNS_80S)}"
        elif r < 0.52:
            # Mot + Suffixe
            word = random.choice(NOUNS_80S if random.random() > 0.4 else PREFIXES_80S)
            raw = f"{word} {random.choice(SUFFIXES_80S)}"
        elif r < 0.72:
            # Collé sans espace (idéal pour les Tones de 10 caractères)
            raw = f"{random.choice(PREFIXES_80S)}{random.choice(SUFFIXES_80S)}"
        elif r < 0.88:
            # Mot simple
            raw = random.choice(NOUNS_80S)
        else:
            # Version dense courte
            raw = f"{random.choice(PREFIXES_80S)[:5]}{random.choice(SUFFIXES_80S)}"

        # Nettoyage + forçage de la longueur exacte
        raw = " ".join(raw.split())          # supprime les espaces multiples
        raw = raw[:length].ljust(length)     # tronque proprement et pad avec espaces
