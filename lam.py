...
class LAMApp:
    ...
    def _is_byte_locked(self, rel_param_offset):
        if 0 <= rel_param_offset <= 25:
            if self.lock_structures.get() and rel_param_offset in (0, 1, 3):
                return True
            if self.lock_bpc.get() and rel_param_offset in (2, 20, 21, 22):
                return True
            if self.lock_lfo.get() and (4 <= rel_param_offset <= 15):
                return True
            if self.lock_fx.get() and (16 <= rel_param_offset <= 19):
                return True
            if self.lock_eq.get() and (23 <= rel_param_offset <= 25):
                return True

        if rel_param_offset >= 26:
            p_offset = (rel_param_offset - 26) % 126
            if self.lock_pitch.get() and (0 <= p_offset <= 14):
                return True
            if self.lock_tvf.get() and (15 <= p_offset <= 32):
                return True
            if self.lock_tva.get() and (33 <= p_offset <= 49):
                return True
            if self.lock_lfo.get() and (p_offset in (3, 17, 35)):
                return True
        ...
        for b_idx in range(1, count + 1):
            ...
            for start, end in messages:
                ...
                for d_idx in range(data_start, checksum_pos):
                    ...
                    if pure_analog and addr_high != 0x00:
                        pass
